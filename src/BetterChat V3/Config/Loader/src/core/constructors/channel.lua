...
return function(network,config,signal)
	...
	local fetchChannelHistoryForSpeaker = function(channel,s)
		...
		for _,poll in pairs(channel.polls) do
			table.insert(receive.polls,poll)
		end
	...
	function channelModule.new(name,autojoin,handlingMessagesOnOwn)
		...
		function channel:postPoll(pollData,thread)
			pollCount += 1
			local id = pollCount
			local relativeMessage = channel.history[#channel.history] or {data = {id = 1}}
			channel.polls[pollCount] = pollData
			removeAt[relativeMessage.data.id] = id
			pollData.relativeId = relativeMessage.data.id
			return function()
				removeAt[relativeMessage.data.id] = nil
				channel.polls[id] = nil
			end
		end
		...
		function channel:sendMessage(speaker,message,replyTo,cb,historyLogic)
			...
			if(#channel.history > messagesLimit) then
				local message = channel.history[#channel.history]
				if(removeAt[message.data.id]) then
					local id = removeAt[message.data.id]
					channel.polls[id] = nil
				end
