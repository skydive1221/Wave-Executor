...
if(currentPlatform ~= "Console") then
	...
	if(not privacy.chatDisabled) then
		...
		local poll = require(elements:WaitForChild("pollCreator"))
		...
		message_senders = {
			...
			poll = require(messages:WaitForChild("poll"))(environment),
		...
		if(environment.config.Polls) then
			if(environment.config.Polls.Enabled) then
				-- create poll:

				function environment:createPoll()
					poll(gui,function(data)
						network:invoke("createPoll",data,environment.currentChannel)
					end)
				end
				
				local onPoll = function(data)
					if(environment.currentChannel == data.channel) then
						message_senders.poll(data)
					end
				end
				
				environment.network.onClientEvent("pollUpdate",function(received)
					onPoll(received)
				end)
				
				handlePolls = function(received)
					for _,poll in pairs(received.polls or {}) do
						task.spawn(onPoll,poll)
					end
				end
			end
		end
